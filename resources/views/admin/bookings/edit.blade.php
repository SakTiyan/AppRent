...
<div class="max-w-2xl mx-auto space-y-6" data-aos="fade-up">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Ubah Data Transaksi</h1>
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">Koreksi atau ubah rincian data peminjaman unit pelanggan.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 transition-colors duration-300">
        <form action="{{ route('admin.bookings.update', $booking->id) }}" method="POST">
            ...
            <div class="mb-5">
                <label for="iphone_id" class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">Pilih Unit iPhone</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                <div>
                    ...
                    <input type="date" name="tanggal_sewa" id="tanggal_sewa" class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-slate-700 focus:ring-2 focus:ring-brand-blue focus:border-brand-blue transition-all outline-none bg-gray-50 dark:bg-slate-950/50 focus:bg-white dark:focus:bg-slate-900 font-medium text-gray-700 dark:text-slate-300 dark:[color-scheme:dark]" value="{{ $booking->tgl_sewa }}" required>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_kembali" id="tanggal_kembali" class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-slate-700 focus:ring-2 focus:ring-brand-blue focus:border-brand-blue transition-all outline-none bg-gray-50 dark:bg-slate-950/50 focus:bg-white dark:focus:bg-slate-900 font-medium text-gray-700 dark:text-slate-300 dark:[color-scheme:dark]" value="{{ $booking->tgl_kembali }}" required>
                </div>
            </div>

            <div class="mb-8">
                <label for="status_booking" class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">Status Pemesanan Saat Ini</label>
                <select name="status_booking" id="status_booking" class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-slate-700 focus:ring-2 focus:ring-brand-blue focus:border-brand-blue transition-all outline-none bg-gray-50 dark:bg-slate-950/50 focus:bg-white dark:focus:bg-slate-900 font-medium text-gray-700 dark:text-slate-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8 bg-slate-50 dark:bg-slate-950/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
                <div>
                    <p class="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Durasi Sewa Baru</p>
                    <div class="text-xl font-bold text-slate-700 dark:text-slate-300"><span id="display_durasi">{{ $booking->total_hari }}</span> Hari</div>
                </div>
                <div>
