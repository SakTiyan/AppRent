...
<div class="max-w-2xl mx-auto space-y-6" data-aos="fade-up">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Tambah Unit iPhone</h1>
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">Masukkan data unit gadget baru yang siap disewakan.</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.iphones.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                <div>
                    ...
                    <input type="text" name="warna" id="warna" class="w-full px-4 py-3 rounded-xl border @error('warna') border-red-400 dark:border-red-500 @else border-gray-200 dark:border-slate-700 @enderror focus:ring-2 focus:ring-brand-blue focus:border-brand-blue transition-all outline-none bg-gray-50 dark:bg-slate-950/50 focus:bg-white dark:focus:bg-slate-900 text-gray-800 dark:text-white" placeholder="Contoh: Black Titanium" value="{{ old('warna') }}" required>
                    @error('warna') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="harga_per_hari" class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">Harga Sewa Per Hari</label>
                    <div class="relative">
                        <span class="absolute left-4 top-3.5 text-gray-400 dark:text-slate-500 font-semibold text-sm">Rp</span>
                        <input type="number" name="harga_per_hari" id="harga_per_hari" class="w-full pl-12 pr-4 py-3 rounded-xl border @error('harga_per_hari') border-red-400 dark:border-red-500 @else border-gray-200 dark:border-slate-700 @enderror focus:ring-2 focus:ring-brand-blue focus:border-brand-blue transition-all outline-none bg-gray-50 dark:bg-slate-950/50 focus:bg-white dark:focus:bg-slate-900 font-semibold text-gray-800 dark:text-white" placeholder="Contoh: 150000" value="{{ old('harga_per_hari') }}" required>
                    </div>
            ...
            <div class="mb-8">
                <label for="gambar" class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">Foto / Gambar iPhone</label>
                <input type="file" name="gambar" id="gambar" class="w-full px-4 py-2.5 rounded-xl border @error('gambar') border-red-400 dark:border-red-500 @else border-gray-200 dark:border-slate-700 @enderror bg-gray-50 dark:bg-slate-950/50 dark:text-slate-300 text-sm file:mr-4 file:py-1.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-brand-blue file:text-white hover:file:bg-blue-700 transition-all outline-none">
                <p class="text-xs text-gray-400 dark:text-slate-500 mt-2">*Format file wajib gambar (jpg, png, jpeg), maksimal 2MB.</p>
