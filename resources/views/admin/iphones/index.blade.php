...
@section('content')
    <div class="space-y-6" data-aos="fade-up">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Kelola Data iPhone</h1>
                <p class="text-sm text-gray-500 mt-1">Daftar unit iPhone inventaris toko yang siap disewakan.</p>
            </div>
                        ...
                        @forelse($iphones as $index => $iphone)
                            <tr class="hover:bg-gray-50/70 transition">
                                ...
                                <td class="py-4 px-6 text-center">
                                    <div class="w-14 h-14 rounded-xl bg-gray-100 flex items-center justify-center mx-auto border border-gray-200 overflow-hidden shadow-sm">
                                        @if ($iphone->gambar)
                                            <img src="{{ asset('uploads/iphones/' . $iphone->gambar) }}" alt="Gambar" class="w-full h-full object-cover">
                                        @else
                                            <i class="fa-solid fa-mobile-screen text-2xl text-gray-300"></i>
                                        @endif
                                    </div>
                                </td>
                                <td class="py-4 px-6 font-bold text-gray-800">{{ $iphone->tipe_iphone }}</td>
                                <td class="py-4 px-6 font-medium">{{ $iphone->kapasitas }} GB</td>
                                <td class="py-4 px-6 font-medium capitalize">{{ $iphone->warna }}</td>
                                <td class="py-4 px-6 font-semibold text-gray-800">Rp {{ number_format($iphone->harga_perhari, 0, ',', '.') }}</td>
                                <td class="py-4 px-6 text-center">
                                    @if ($iphone->status == 'Tersedia')
                                        <span class="px-3 py-1.5 bg-green-50 text-green-600 rounded-full text-xs font-bold uppercase tracking-wide">Tersedia</span>
                                    @elseif($iphone->status == 'Disewa')
                                        <span class="px-3 py-1.5 bg-yellow-50 text-yellow-600 rounded-full text-xs font-bold uppercase tracking-wide">Disewa</span>
                                    @else
                                        <span class="px-3 py-1.5 bg-red-50 text-red-600 rounded-full text-xs font-bold uppercase tracking-wide">Maintenance</span>
                                    @endif
                                </td>
                                <td class="py-4 px-6 text-center flex items-center justify-center gap-2 mt-2">
                                    <a href="{{ route('admin.iphones.show', $iphone->id) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Lihat Detail">
                                        <i class="fa-solid fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.iphones.edit', $iphone->id) }}" class="p-2 text-amber-600 hover:bg-amber-50 rounded-lg transition" title="Edit">
