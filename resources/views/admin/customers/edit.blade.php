...
<div class="max-w-2xl mx-auto space-y-6" data-aos="fade-up">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Edit Data Customer</h1>
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">Ubah atau perbarui informasi data identitas pelanggan.</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.customers.update', $customer->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-4 top-3.5 text-gray-400 dark:text-slate-500"><i class="fa-brands fa-whatsapp text-base"></i></span>
                        <input type="text" name="no_hp" id="no_hp" class="w-full pl-10 pr-4 py-3 rounded-xl border @error('no_hp') border-red-400 dark:border-red-500 @else border-gray-200 dark:border-slate-700 @enderror focus:ring-2 focus:ring-brand-blue focus:border-brand-blue transition-all outline-none bg-gray-50 dark:bg-slate-950/50 focus:bg-white dark:focus:bg-slate-900 text-gray-800 dark:text-white" value="{{ old('no_hp', $customer->no_hp) }}" required>
                    </div>
            ...
            <div class="mb-5">
                <label for="nama_lengkap" class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">Nama Lengkap</label>
                <input type="text" name="nama_lengkap" id="nama_lengkap" class="w-full px-4 py-3 rounded-xl border @error('nama_lengkap') border-red-400 dark:border-red-500 @else border-gray-200 dark:border-slate-700 @enderror focus:ring-2 focus:ring-brand-blue focus:border-brand-blue transition-all outline-none bg-gray-50 dark:bg-slate-950/50 focus:bg-white dark:focus:bg-slate-900 text-gray-800 dark:text-white" value="{{ old('nama_lengkap', $customer->nama_lengkap) }}" required>
            ...
            <div class="mb-8">
                ...
                <textarea name="alamat" id="alamat" rows="3" class="w-full px-4 py-3 rounded-xl border @error('alamat') border-red-400 @else border-gray-200 @enderror focus:ring-2 focus:ring-brand-blue focus:border-brand-blue transition-all outline-none bg-gray-50 dark:bg-slate-950/50 focus:bg-white dark:focus:bg-slate-900 text-gray-800 dark:text-white resize-none" required>{{ old('alamat', $customer->alamat) }}</textarea>
